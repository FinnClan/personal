...
abstract public class Persistable {
	...
	/**
	 * Create a Properties object representing aspects of the 'schema' of a table -
	 * namely, the column names and the types of the columns
	 *
	 * @param String
	 *            Table name to get schema information for
	 *
	 * @return Properties object indicating column names as keys and column types as
	 *         values
	 */
	// ------------------------------------------------------------
	protected Properties getSchemaInfo(String tableName) {
		...
		try {
			...
			/*
			 * System.out.println("Persistable.getSchemaInfo(..) connection = " +
			 * theDBConnection);
			 */
			...
			while (columnInfo.next()) {
				// DEBUG System.out.println("Column Name = " + columnInfo.getString(4) + ",
				// Column Type = " + columnInfo.getString(6));
	...
	/**
	 * Create and execute a SQL statement to extract the required fields from the
	 * database. Returns a Vector with each element being a Properties object
	 * containing the columnName=columnValue mappings
	 */
	// ------------------------------------------------------------
	protected Vector getPersistentState(Properties schema, Properties where) {
		...
		try {
			...
			// new Event(Event.getLeafLevelClassName(this), "getPersistentState",
			// "Attempting to get database connection", Event.DEBUG);

			theDBConnection = myBroker.getConnection();

			// new Event(Event.getLeafLevelClassName(this), "getPersistentState", "Return
			// from database connection attempt call",
			// Event.DEBUG);
	...
	protected Vector getQueriedState(Properties selSchema, Properties projectionSchema, Properties where) {
		...
		try {
			...
			// DEBUG: System.out.println("SQLQueryStatement: " +
			// theSQLStatement.toString());
			...
			System.err.println("Persistable.getQueriedState: An SQL Error Occurred:" + sqle + "\n" + sqle.getErrorCode()
					+ "\n" + sqle.getMessage() + "\n" + sqle);
			new Event(Event.getLeafLevelClassName(this), "getQueriedState",
					"SQL Exception: " + sqle.getErrorCode() + ": " + sqle.getMessage(), Event.ERROR);
			return null;
		} finally {
	 ...
	 */
	// ------------------------------------------------------------
	protected Vector getQueriedStateWithExactMatches(Properties selSchema, Properties projectionSchema,
			Properties where) {
		int numRSColumns = 0; // number of columns in ResultSet
		Vector namesRSColumns = null; // names of columns in ResultSet
		ResultSet theResultSet; // the resultset from the SQLStatement execution
		...
		try {
			...
			// construct a SQL statement from the passed parameters
			SQLQueryStatementWithExactMatches theSQLStatement = new SQLQueryStatementWithExactMatches(selSchema,
					projectionSchema, where);
	 ...
	 * the required fields from the database. Returns a Vector with each element
	 * being a Properties object containing the columnName=columnValue mappings
	 */
	// ------------------------------------------------------------
	...
	/**
	 * Create and execute a SQL statement to extract the required fields from the
	 * database. Returns an int indicating the return code from SQL UPDATE statement
	 */
	// ------------------------------------------------------------
	protected Integer updatePersistentState(Properties schema, // the table schema
		...
		try {
			...
			System.err.println("An SQL Error Occured:" + sqle + "\n" + sqle.getErrorCode() + "\n" + sqle.getMessage()
					+ "\n" + sqle);
			new Event(Event.getLeafLevelClassName(this), "updatePersistentState",
					"SQL Exception: " + sqle.getErrorCode() + ": " + sqle.getMessage(), Event.ERROR);
			throw sqle;
		} finally {
			closeStatement();
	...
	/**
	 * Create and execute a SQL statement to insert the required fields into the
	 * database. Returns an int indicating the auto-incremental id from SQL INSERT
	 * statement
	 */
	// ------------------------------------------------------------
	protected Integer insertAutoIncrementalPersistentState(Properties schema, // the table schema
		...
		try {
			...
			// DEBUG System.out.println("Persistable.insertPersistentState - SQL Statement:
			// " + theSQLStatement.toString());
			...
			if (theResultSet.next()) {
				...
				System.out
						.println("Persistable.insertAutoIncrementalPersistentState - can't get the auto-increment key");
			}

			return new Integer(autoIncKey);
		} catch (SQLException sqle) {
			// DEBUG: System.err.println( "Persistable.insertAutoIncrementalPersistentState:
			// An SQL Error Occurred: SQL State: " + sqle.getSQLState() + "; Error Code = "
			// + sqle.getErrorCode() + "; Message: " + sqle.getMessage() + "\n" + sqle);
	...
	/**
	 * Create and execute a SQL statement to insert the required fields into the
	 * database. Returns an int indicating the return code from SQL INSERT statement
	 */
	// ------------------------------------------------------------
...

	{
		...
		try {
			...
			System.err.println("An SQL Error Occurred:" + sqle + "\n" + sqle.getErrorCode() + "\n" + sqle.getMessage()
					+ "\n" + sqle);
			new Event(Event.getLeafLevelClassName(this), "insertPersistentState",
					"SQL Exception: " + sqle.getErrorCode() + ": " + sqle.getMessage(), Event.ERROR);
			throw sqle;
		} finally {
			closeStatement();
	...
	/**
	 * Create and execture a SQL statement to delete the required fields from the
	 * database. Returns an integer indicating the return code from SQL DELETE
	 * statement
	 */
	// ------------------------------------------------------------
	protected Integer deletePersistentState(Properties schema, // the table schema
		...
		try {
			...
			// DEBUG System.out.println("Persistable.deletePersistentState - SQL Statement:
			// " + theSQLStatement.toString());
			...
			System.err.println("An SQL Error Occured:" + sqle + "\n" + sqle.getErrorCode() + "\n" + sqle.getMessage()
					+ "\n" + sqle);
			new Event(Event.getLeafLevelClassName(this), "deletePersistentState",
					"SQL Exception: " + sqle.getErrorCode() + ": " + sqle.getMessage(), Event.ERROR);
			throw sqle;
		} finally {
			closeStatement();
